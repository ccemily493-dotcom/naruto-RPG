...
export class GeminiFlashProvider implements IGMProvider {
  public id: GMProviderId = 'gemini-flash';
  public name = 'Gemini Flash';
  public isFree = true;
  ...
  public async generateStream(
    ...
    if (!apiKey) {
      throw new Error('GEMINI_FLASH_UNAVAILABLE: Configuración incompleta. Falta GEMINI_API_KEY en .env');
    }
    ...
    try {
      ...
      if (msg.includes('429') || msg.includes('quota') || msg.includes('RESOURCE_EXHAUSTED')) {
        const error = new Error(`Gemini Flash Quota/Rate Limit Exhausted: ${msg}`);
